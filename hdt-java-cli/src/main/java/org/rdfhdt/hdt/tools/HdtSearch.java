...
public class HdtSearch implements ProgressListener {
    ...
    protected static void iterate(final HDTPrivate hdt, CharSequence subject, CharSequence predicate, CharSequence object) throws NotFoundException {
	...
	final IteratorTripleString it = hdt.search(subject,predicate,object);
	count = 0;
	while(it.hasNext()) {
	...
	try {
	    final TripleString triplePattern = new TripleString();
