 ...
 *
 */
public class HashGraphsDictionary extends BaseTempGraphsDictionary {

    public HashGraphsDictionary(final HDTOptions spec) {
	super(spec);
	...
	this.graphs = new HashDictionarySection();
    }
	...
	final DictionaryIDMapping mapGraph = new DictionaryIDMapping(this.graphs.getNumberOfElements());
	...
	st.reset();
	final Iterator<? extends CharSequence> itPred = this.graphs.getEntries();
	...
	for (int j = 0; j < mapGraph.size(); j++) {
	    ...
	    // System.out.print("Graph Old id: "+(j+1) + " New id: "+ mapPred.getNewID(j)+ " STR: "+mapPred.getString(j));
	}
	...
	while (iteratorTriples.hasNext()) {
	    ...
	    triples.update(triple,
		    ...
		    mapGraph.getNewID(triple.getPredicate() - 1),
	...
	return this.subjects.getNumberOfElements() + this.graphs.getNumberOfElements() + this.objects.getNumberOfElements() + this.shared.getNumberOfElements();
    }
